...
SRC_LANG    = "eng_Latn"
TGT_LANG    = "vie_Latn"
INPUT_JSON  = "wp2v301_mt_tokens_nllb.json"
OUTPUT_JSON = "wp4v302_mt_ig_nllb_norm.json"
N_STEPS     = 24

# "sum" | "l1" | "l2"
# sum: tổng qua chiều embedding (có thể âm, giữ hướng)
# l1 : tổng trị tuyệt đối (luôn dương, đo độ lớn tổng)
# l2 : căn bậc hai tổng bình phương (chuẩn Euclidean, nhạy với outlier hơn l1)
AGGREGATION = "l2"
...
def compute_ig(item: dict) -> dict | None:
    ...
    # Aggregate over embedding dim → scalar per source token
    # sum : có thể âm — dương nghĩa là ủng hộ, âm nghĩa là kìm hãm output
    # l1  : tổng trị tuyệt đối — đo tổng độ lớn attribution, bỏ hướng
    # l2  : chuẩn Euclidean — nhạy hơn với token có một vài chiều attribution rất lớn
    if AGGREGATION == "sum":
        raw_scores = attributions.sum(dim=-1).squeeze(0).detach().cpu().numpy()
    elif AGGREGATION == "l1":
        raw_scores = attributions.abs().sum(dim=-1).squeeze(0).detach().cpu().numpy()
    elif AGGREGATION == "l2":
        raw_scores = attributions.norm(p=2, dim=-1).squeeze(0).detach().cpu().numpy()
    else:
        raise ValueError(f"AGGREGATION phải là 'sum', 'l1', hoặc 'l2'. Nhận: {AGGREGATION}")
